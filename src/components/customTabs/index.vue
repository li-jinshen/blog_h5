	...
	<view class="_tab-box" :style="{fontSize: defaultConfig.fontSize + 'rpx', color: defaultConfig.color}">
		<scroll-view id="_scroll" :scroll-x="true" class="scroll-view-h" scroll-with-animation
			...
			<view class="_scroll-content">
				<view class="_tab-item-box" :class="[defaultConfig.itemWidth ? '_clamp' : '_flex',useFlex?'flex':'']">
					<block v-for="(item, index) in tabList" :key="index">
						<view class="_item" :id="'_tab_'+index"
							:class="{ '_active': tagIndex === index ,'flex-auto':useFlex}"
							...
							:sytle="{fontSize:defaultConfig.fontSize ?defaultConfig.fontSize +'rpx':'28rpx','font-weight':defaultConfig.blod?'bold':''}">
							{{ item[defaultConfig.key]?item[defaultConfig.key] :"未知项"}}
						</view>
					</block>
				</view>
